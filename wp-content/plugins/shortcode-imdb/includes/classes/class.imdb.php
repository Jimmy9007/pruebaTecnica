...
//	    return $response['body'];
    ...
    function title($id){
	    ...
	    @$error_bubble  = $xpath->query('//div[@class="error_bubble"]');
	    $yeni = false;
        $dJ = $xpath->query( '//script[@type="application/json"]');
        $encJ = json_decode($dJ->item(0)->nodeValue);
	    if(!isset($error_bubble->item(0)->nodeValue)) {
		    $data['error'] = "no";
		    /* Poster */
		    ...
		    if ( isset( $title->item( 0 )->nodeValue ) ) {
			    ...
			    $data["title"] = sanitize_text_field( trim( $tpar . utf8_decode($tt[0]), chr( 0xC2 ) . chr( 0xA0 ) ) );
		    ...
		    if($yeni){
			    ...
			    $aka         = sanitize_text_field(utf8_decode($aka->item( 0 )->nodeValue ));
			    $data['aka'] = $aka;
		    }
			    ...
			    if ( isset( $year ) ) {
				    foreach ( $year as $y ) {
					    ...
					    if ( strpos( $check, 'releaseinfo' ) !== false ) {
						    $data['year'] = isset( $y->nodeValue ) ? sanitize_text_field( trim( $y->nodeValue ) ) : "";
					    }
				    }
			    }
		    ...
		    if($yeni){
			    ...
			    $data['year'] = isset( $year->item( 0 )->nodeValue ) ? sanitize_text_field(trim( substr( $year->item( 0 )->nodeValue, 0, 4 ) ) ) : "";
		    }
		    ...
		    if ( isset( $genre ) ) {
			    ...
			    foreach ( $genre as $g ) {
				    ...
				    if ( strpos( $check_genre, 'genres=' ) !== false ) {
					    $genres[ $x ] = sanitize_text_field( trim( $g->nodeValue ) );
					    $x ++;
				    }
			    }
		    ...
		    /* Rating */
		    $rating = $encJ->props->pageProps->aboveTheFoldData->ratingsSummary->aggregateRating;
		    ...
		    $data['votes'] = isset( $votes->item( 0 )->nodeValue ) ? sanitize_text_field( trim( $votes->item( 0 )->nodeValue ) ) : "";
		    if($yeni){
		    ...
		    $data['metascore'] = isset( $meta->item( 0 )->nodeValue ) ? sanitize_text_field(( trim( $meta->item( 0 )->nodeValue ) )): "";
		    if($yeni){
		    ...
		    if($yeni){
			    ...
			    $data['release'] = isset( $release->item( 0 )->nodeValue ) ? sanitize_text_field(( trim( $release->item( 0 )->nodeValue ) )) : "";
		    }

		    /* Summary */
		    @$sum = $xpath->query( '//div[@class="summary_text"]' );
		    @$data['sum'] = isset( $sum->item( 0 )->nodeValue ) ? sanitize_text_field(( trim( $sum->item( 0 )->nodeValue ) )) : "";
		    if($yeni){
		    ...
		    if ( isset( $budget ) ) {
			    foreach ( $budget as $b ) {
				    if (str_contains($b->nodeValue, 'Budget')) {
					    $mybudget = trim( str_replace( "Budget:", "", $b->nodeValue ) );
				    } else if (str_contains($b->nodeValue, 'Gross')) {
...
//
		    ...
		    $data['budget'] = sanitize_text_field( $mybudget );
		    $data['gross']  = sanitize_text_field( $mygross );
		    ...
		    if($yeni){
		    	...
		    	if(@$videos->length>0){
				    ...
				    foreach ( $videos as $z=> $v ){
                        ...
                        if(isset($vidlink->item($z)->nodeValue)) {
...
//				     $wpdb->query( 'INSERT INTO ' . $wpdb->prefix . 'shortcode_imdb_cache (imdb_id,title,type,cache) VALUES ("' . $id . '","' . $Title . '","list","' . $arr . '")' );
	...
	function grab_imdb($id,$type){
		...
		if(count($result)>0){
			...
			if($type=="name"){
				$values = $this->name($id);
				$title = sanitize_text_field($values->name);
			}elseif($type=="title"){
